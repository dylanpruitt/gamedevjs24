...
func _ready():
	...
	for found_item in GlobalVariables.player_found_items:
		...
		var total_item_value = num_found * item_value

		add_item_label("%s ($%s)" % [found_item, item_value], 280, 216 + i*32)
		add_item_label("x%d" % num_found, 460, 216 + i*32)
		add_item_label(" = $%d" % total_item_value, 560, 216 + i*32)
	...
	await get_tree().create_timer(1.0).timeout
	add_item_label("Total found", 280, 216 + (i+1)*32)
	add_item_label(" = $%s" % total_value, 560, 216 + (i+1)*32)
	$TypewriterClickSFX.play()
